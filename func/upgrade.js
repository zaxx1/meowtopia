...
exports.upgradeAnimal = async (value, type, token) => {
  try {
    const getPet = await axios.get("https://api.catopia.io/api/v1/players/pet?limit=3000", {
      headers: {
    ...
    if (Number(gold) > 500000 || Number(pet) <= 3000) {
      for (let index = 0; index < value; index++) {
        const buy = await axios.post("https://api.catopia.io/api/v1/store/buy", {
          storeId: 4,
          price: 60000,
          unit: 1,
        }, {
        ...
        console.log(`[ ${getCurrentTime()} ] [ BUY ] : buy box has been created : length : ${box.length} `);

        const openBox = await axios.post("https://api.catopia.io/api/v1/chest/open-multiple", {
          petTypeIds: [type],
        ...
        console.log(`[ ${getCurrentTime()} ] [ OPEN ] : open box has been created : length : ${boxReadyOpen.length} `);

        const upgrade = await axios.post("https://api.catopia.io/api/v1/players/pet/fast-upgrade", {
          level: 1,
        ...
        await delay(2); // Corrected the delay time to 2 seconds
